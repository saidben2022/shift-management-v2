...
import { addWeeks, startOfWeek, addDays, getISOWeek, startOfYear } from "date-fns";
...
export function getPeriods(year) {
    // Check cache first
...
export function getCurrentPeriod(date = new Date()) {
    ...
    return periods.findIndex(period => timestamp >= period.start.getTime() && timestamp <= period.end.getTime());
}
export function formatDate(date) {
    // Format to UTC date string and ensure time is set to start of day
    return date.toISOString().split('T')[0] + 'T00:00:00.000Z';
}
export function getPeriodsAlternative(year) {
